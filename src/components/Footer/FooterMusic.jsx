...
export default function FooterMusic() {
    // State за текущото време на песента
    ...
    // Симулираме времето на песента – тук с интервал, който обновява currentTime на всяка секунда
    useEffect(() => {
        if (isDragging) return; // ако в момента влачим топчето, не обновявай времето
            ...
            >
                ...
                <div className="flex flex-col items-center relative w-[40%] sm:w-[50%] md:w-[60%] max-w-[500px]">
                    <div className="w-full flex justify-center space-x-2 sm:space-x-3">
                        <button className={`${btnClass} text-xs sm:text-sm hover:scale-105 transition-transform duration-200`}>
                            <ShuffleIcon width={18} height={18} />
                        </button>
                        <button className={`${btnClass} text-xs sm:text-sm hover:scale-105 transition-transform duration-200`}>
                            <SkipBackIcon width={18} height={18} />
                        </button>
                        <button className={`${btnClass} text-lg sm:text-xl hover:scale-110 transition-transform duration-200`}>
                            <PauseIcon width="36" height="36" />
                            {/* <PlayIcon width="36" height="36" /> */}
                        </button>
                        <button className={`${btnClass} text-xs sm:text-sm hover:scale-105 transition-transform duration-200`}>
                    ...
                    <div className="absolute -bottom-3 left-0 right-0 px-2 sm:px-6">
                        <div className="flex items-center justify-between w-full">
                            <span className="text-xs sm:text-sm mr-1.5">{formatTime(currentTime)}</span>
                            <div
                                ref={progressBarRef}
                            ...
                            >
                                ...
                                {/* Топче за drag (knob) */}
                                <div
                                    className="absolute -top-1 w-3 h-3 bg-blue-600 rounded-full cursor-pointer"
                                    style={{
                            ...
                            <span className="text-xs sm:text-sm ml-1.5">{formatTime(duration)}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-end space-x-2 sm:space-x-3 min-w-[120px]">
                    <button className={btnClass}>
                        ...
                        {volume > 0 && volume < 1 / 3 && <VolumeIcon className="w-6 h-6" />}
                        {volume >= 1 / 3 && volume < 2 / 3 && <Volume1Icon className="w-6 h-6" />}
                    ...
                    <button className={`${btnClass} hover:scale-105 transition-transform duration-200`}>
                        <SquareMenuIcon className="w-5 h-5" />
                    </button>
                </div>
            </footer>
        </>
